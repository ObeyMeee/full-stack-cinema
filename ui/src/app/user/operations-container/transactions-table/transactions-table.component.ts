...
export class TransactionsTableComponent implements OnChanges, OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['onlyActiveTickets']) {
      this.ticketDtos$ = this.onlyActiveTickets
        ? this.ticketService.getAllActive()
        : this.ticketService.getAll();
    }

    this.ticketDtos$.subscribe((ticketDtos) => {
      this.filmTransactions = ticketDtos
        .reduce((filmTransactions: FilmTransaction[], cur) => {
  ...
  filterTicketsByTransaction(
    ticketDtos: TicketDto[],
    transaction: FilmTransaction,
  ) {
